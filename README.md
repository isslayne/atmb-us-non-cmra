...
ATMB 的每次请求（含重试）默认等待 1500 ms，可用 `ATMB_INTERVAL_MS` 调整。只限制并发数还不够：低延迟的云端 Runner 即使顺序请求也会形成高频突发，导致源站返回缓存的目录重定向。首轮仍失败的详情会在冷却后单独重试最多两轮；详情抓取结束后才查询 Smarty / USPS，避免因重试重复消费 Smarty 额度。
